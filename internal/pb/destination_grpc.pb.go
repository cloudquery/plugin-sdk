...
type DestinationClient interface {
	...
	Close(ctx context.Context, opts ...grpc.CallOption) (Destination_CloseClient, error)
}
...
func (c *destinationClient) Close(ctx context.Context, opts ...grpc.CallOption) (Destination_CloseClient, error) {
	stream, err := c.cc.NewStream(ctx, &Destination_ServiceDesc.Streams[1], "/proto.Destination/Close", opts...)
	if err != nil {
		return nil, err
	}
	x := &destinationCloseClient{stream}
	return x, nil
}

type Destination_CloseClient interface {
	Send(*Close_Request) error
	CloseAndRecv() (*Close_Response, error)
	grpc.ClientStream
}

type destinationCloseClient struct {
	grpc.ClientStream
}

func (x *destinationCloseClient) Send(m *Close_Request) error {
	return x.ClientStream.SendMsg(m)
}

func (x *destinationCloseClient) CloseAndRecv() (*Close_Response, error) {
	if err := x.ClientStream.CloseSend(); err != nil {
		return nil, err
	}
	m := new(Close_Response)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
	return m, nil
...
type DestinationServer interface {
	...
	// Send signal to flush and close open connections
	Close(Destination_CloseServer) error
...
func (UnimplementedDestinationServer) Close(Destination_CloseServer) error {
...
func _Destination_Close_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(DestinationServer).Close(&destinationCloseServer{stream})
}

type Destination_CloseServer interface {
	SendAndClose(*Close_Response) error
	Recv() (*Close_Request, error)
	grpc.ServerStream
}

type destinationCloseServer struct {
	grpc.ServerStream
}

func (x *destinationCloseServer) SendAndClose(m *Close_Response) error {
	return x.ServerStream.SendMsg(m)
}

func (x *destinationCloseServer) Recv() (*Close_Request, error) {
	m := new(Close_Request)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
