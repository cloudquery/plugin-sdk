...
type SourceClient interface {
	...
	// Get metrics for the source plugin. This is a streaming version of the original GetMetrics.
	GetMetrics2(ctx context.Context, in *GetSourceMetrics2_Request, opts ...grpc.CallOption) (Source_GetMetrics2Client, error)
}
...
func (c *sourceClient) GetMetrics2(ctx context.Context, in *GetSourceMetrics2_Request, opts ...grpc.CallOption) (Source_GetMetrics2Client, error) {
	stream, err := c.cc.NewStream(ctx, &Source_ServiceDesc.Streams[2], "/proto.Source/GetMetrics2", opts...)
	if err != nil {
		return nil, err
	}
	x := &sourceGetMetrics2Client{stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
type Source_GetMetrics2Client interface {
	Recv() (*GetSourceMetrics2_Response, error)
	grpc.ClientStream
}

type sourceGetMetrics2Client struct {
	grpc.ClientStream
}

func (x *sourceGetMetrics2Client) Recv() (*GetSourceMetrics2_Response, error) {
	m := new(GetSourceMetrics2_Response)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
	return m, nil
...
type SourceServer interface {
	...
	// Get metrics for the source plugin. This is a streaming version of the original GetMetrics.
	GetMetrics2(*GetSourceMetrics2_Request, Source_GetMetrics2Server) error
...
func (UnimplementedSourceServer) GetMetrics2(*GetSourceMetrics2_Request, Source_GetMetrics2Server) error {
	return status.Errorf(codes.Unimplemented, "method GetMetrics2 not implemented")
}
...
func _Source_GetMetrics2_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(GetSourceMetrics2_Request)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(SourceServer).GetMetrics2(m, &sourceGetMetrics2Server{stream})
}

type Source_GetMetrics2Server interface {
	Send(*GetSourceMetrics2_Response) error
	grpc.ServerStream
}

type sourceGetMetrics2Server struct {
	grpc.ServerStream
}

func (x *sourceGetMetrics2Server) Send(m *GetSourceMetrics2_Response) error {
	return x.ServerStream.SendMsg(m)
}
...
var Source_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "GetMetrics2",
			Handler:       _Source_GetMetrics2_Handler,
			ServerStreams: true,
		},
	},
