...
func TestTableFromGoStruct(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, col := range tt.want.Columns {
				got := table.Columns.Get(col.Name)
				if got == nil {
					t.Errorf("column %s expected but not found", col.Name)
					continue
				}
				if !arrow.TypeEqual(got.Type, col.Type) {
					t.Errorf("column %s does not match expected type. got %v, want %v", col.Name, got.Type, col.Type)
				}
			}
			if diff := cmp.Diff(table.Columns, tt.want.Columns,
				cmpopts.IgnoreFields(schema.Column{}, "Resolver"),
				cmpopts.IgnoreFields(schema.Column{}, "Type")); diff != "" {
