...
func (*DefaultReverseTransformer) ReverseTransformValues(table *schema.Table, values []interface{}) (cqtypes.CQTypes, error) {
	valuesSlice := helpers.InterfaceSlice(values)
	res := make(cqtypes.CQTypes, len(valuesSlice))

	for i, v := range valuesSlice {
		t := schema.CQTypeFromSchema(table.Columns[i].Type)
